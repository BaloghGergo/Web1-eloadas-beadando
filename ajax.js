...
async function calculateHeightStats() {
  let response = await fetch(url, {
      method: 'post',
      cache: 'no-cache',
      headers: {
  ...
  if (list.length === 0) {
    document.getElementById("statsResult").innerHTML = "No data available!";
    return;
  }

  let sum = 0;
  let maxHeight = Number(list[0].height);

  for (let i = 0; i < list.length; i++) {
    let h = Number(list[i].height);
    sum += h;
  ...
  let resultStr = "<h2>Magasság statisztika</h2>";
  resultStr += "<p>Összeg: " + sum.toFixed(2) + "</p>";
  resultStr += "<p>Átlag: " + average.toFixed(2) + "</p>";
  resultStr += "<p>Maximum: " + maxHeight.toFixed(2) + "</p>";

  document.getElementById("statsResult").innerHTML = resultStr;
}
